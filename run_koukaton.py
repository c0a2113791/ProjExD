...
#スクリーンサイズ（800,200）を生成
screen_size_display = (width_screen, height_screen) = (800, 200)
FPS = 80
gravity = 0.6
...
pygame.display.set_caption("koukaton Run ")
...
class koukaton():
    ...
    def __init__(self, sx=-1, sy=-1):  #こうかとんのコマ送り画像配置（予定）
        self.imgs, self.rect = load_sprite_sheet('koukaton (2).png', 5, 1, sx, sy, -1)
        self.imgs1, self.rect1 = load_sprite_sheet('koukaton (2).png', 2, 1, 59, sy, -1)  #（手直し）画像の縮尺等が違うため画像が荒ぶる
...
def introduction_screen():
    ado_koukaton = koukaton(44,47)
    ado_koukaton.blinking = True
    starting_game = False
    ...
    logo,l_rect = load_image('runkoukaton.png',300,160,-1)
    ...
    while not starting_game:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_SPACE or event.key == pygame.K_UP:
                        ado_koukaton.jumping = True
                        ado_koukaton.blinking = False
                        ado_koukaton.movement[1] = -1*ado_koukaton.jumpSpeed
        ...
        time_clock.tick(FPS)
        if ado_koukaton.jumping == False and ado_koukaton.blinking == False:
            starting_game = True
...
def gameplay():
    ...
    while not g_exit:
        ...
        while not g_Over:
                ...
                for event in pygame.event.get():
                    ...
                    if event.type == pygame.KEYDOWN:
                        if event.key == pygame.K_SPACE:
                            if gamer_koukaton.rect.bottom == int(0.98 * height_screen):
                                gamer_koukaton.jumping = True
                                if pygame.mixer.get_init() != None:
                                    jump_sound.play()
                                gamer_koukaton.movement[1] = -1*gamer_koukaton.jumpSpeed

                        if event.key == pygame.K_DOWN:
                            if not (gamer_koukaton.jumping and gamer_koukaton.dead):
                                gamer_koukaton.ducking = True

                    if event.type == pygame.KEYUP:
                        if event.key == pygame.K_DOWN:
                            gamer_koukaton.ducking = False
            for c in cactusan:
                c.movement[0] = -1*gp
                if pygame.sprite.collide_mask(gamer_koukaton,c):
                    gamer_koukaton.dead = True
                    if pygame.mixer.get_init() != None:
                        die_sound.play()

            for p in smallBird:
                p.movement[0] = -1*gp
                if pygame.sprite.collide_mask(gamer_koukaton,p):
                    gamer_koukaton.dead = True
                    if pygame.mixer.get_init() != None:
                        die_sound.play()
            ...
            gamer_koukaton.update()
            cactusan.update()
            smallBird.update()
            skyClouds.update()
            new_grnd.update()
            score_boards.update(gamer_koukaton.score)
            ...
            if gamer_koukaton.dead:
                g_Over = True
                if gamer_koukaton.score > highest_scores:
                    highest_scores = gamer_koukaton.score
